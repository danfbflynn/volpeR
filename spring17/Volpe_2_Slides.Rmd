...
For example, perhaps the course is an upper-level couse, and freshmen are expected to be only 15% of the registered students. We can specify the expected proprotion as a vector `p`
...
```
## Regression
There is a lot to unpack with these results. Recall that regression minimizes the sum of squares bewteen the observed values and the expected values. This method is called *Ordinary Least Squares (OLS)*, finding the slope coefficient which best matches the predictor and response variables.
...
## Regression 
We can use these coeffients to test scenarios. For example what if NOx concentrations are reduced by half?
...
## Exercise 2.2
How do we compare models? If we have the same response data in each model, we can compare using an index called AIC, Akaike Informaiton Criterion. Of course you can look at $R^{2}$ as well, but there are pitfalls with only looking at $R^{2}$. 
